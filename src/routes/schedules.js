...
app.post("/:scheduleId/update", ensureAuthenticated(), scheduleFormValidator, async (c) => {
  const { user } = c.get("session") ?? {};
  const schedule = await prisma.schedule.findUnique({
    where: { scheduleId: c.req.valid("param").scheduleId },
  });
...
app.post("/:scheduleId/delete", ensureAuthenticated(), scheduleFormValidator, async (c) => {
  const { user } = c.get("session") ?? {};
  const schedule = await prisma.schedule.findUnique({
    where: { scheduleId: c.req.valid("param").scheduleId },
  });
